...
<?php if ($data['bIsCommentloadMore'] > 0): ?>
        ...
        <?php if (empty($blockCommentId)): ?>
            <?php $this->Html->viewMoreChrono($data['more_comments'].'/is_close_comment:'.$is_close_comment.$more_link,'comments') ?>
        <?php else: ?>
            <?php $this->Html->viewMoreChrono($data['more_comments'].'/is_close_comment:'.$is_close_comment.'/id_content:'.$blockCommentId.$more_link,$blockCommentId) ?>
        <?php endif; ?>
            ...
            <ul class="item_comments comment_list <?php echo !empty($comment['Replies']) ? 'isLoadNew' : '';?>" id="item_comments_reply_<?php echo $comment['Comment']['id']?>">
                <?php if(!empty($comment['RepliesIsLoadMore']) && $comment['RepliesIsLoadMore']):?>
                    ...
                    <?php if(!empty($comment['Replies'])):
                                            ...
                                            ?>
                    <?php echo $this->element('comments_chrono', array('data' => $reply_data, 'uid' => $uid, 'blockCommentId' => $blockCommentId));?>
                <?php endif;?>
                ...
                <?php if ($comment['Comment']['count_reply']):?>
                <li>
