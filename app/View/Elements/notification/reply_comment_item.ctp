...
if ($params['type'] == 'Photo_Photo')
{
	...
	{
		$title = __('replied to your comment on your photo');
	}
	else
	{
		$title = __('replied to your comment on %s photo',possession( $notification['Sender'], $object['User']));
	}
}
else
{
	$title = __('replied to your comment on').' "'.$object[key($object)]['moo_title'].'"';
}
?>
<?php echo $title;?>