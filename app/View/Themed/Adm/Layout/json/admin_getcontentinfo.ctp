            ...
            foreach($block_info as $key=> $value){
                ...
                if($value['input'] == 'radio' || $value ['input'] == 'checkbox' || $value ['input'] == 'select'){
                    ...
                    if ($value['input'] == 'hidden'){
                        ...
                        $tip = array();
                        $convertValue;
                        if($value['name'] == 'title' || $value['name'] == 'menu_id' || $value['name'] == 'plugin'){
                            //$disable = 'disabled';
                            $convertValue = $data[0]['CoreContent']['blockName'];
                            if($value['name'] == 'title')
                            {
                                if(!empty($data[0]['CoreContent']['contentId']))
                                    $tip = array('after' => '</div><div class="tips" style="margin-left: 165px;"><a data-dismiss="modal" href="'.$this->request->base.'/admin/core_contents/ajax_translate/'.$data[0]['CoreContent']['contentId'].'"  data-toggle="modal" data-target="#ajax-translate2" >'.__('Translation').'</a></div>');
                                else
