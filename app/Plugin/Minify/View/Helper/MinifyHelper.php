 ...
 * @var array
 */
	public $helpers = array('Html','Storage.Storage');
	public $_paths = array();
 ...
 */
	...
	public function addPath($paths)
	{
		...
		if (!filter_var($paths, FILTER_VALIDATE_URL) === false)
			return;


		$this->_paths[] = substr($this->Html->assetUrl($paths, array('ignoreCDN'=>true,'pathPrefix' => Configure::read('App.cssBaseUrl'), 'ext' => '.css')),1);
	}

	public function render()
	{
		// krsort($this->_paths);
		$paths = array_unique($this->_paths);
		if (!count($paths))
			return;

		$minifyModel = MooCore::getInstance()->getModel("Minify.MinifyUrl");
		$name = md5(json_encode($paths));
		$minify = $minifyModel->getMinify($name,$paths);


		//return '<link rel="stylesheet" type="text/css" href="'.$this->_View->request->base.'/min-css?f='.$name.'">';
		return '<link rel="stylesheet" type="text/css" href="'.$this->Storage->getUrl($minify['MinifyUrl']['id'],'',$minify['MinifyUrl']['name'],"minify").'?v='.Configure::read('core.link_version').'">';
	}
}
