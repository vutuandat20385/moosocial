...
<form method="post">
	...
	<div class="table-compare">
		<table class="hidden-xs">
			<tr>
				<td><?php echo __('Choose a Package');?></td>
				<?php foreach ($columns as $column): if (!count($column['SubscriptionPackagePlan'])) continue;?>
					<?php $package_exit[] = $column;?>
					<td class="<?php if ($column['SubscriptionPackage']['recommended']): ?>plan-recommend<?php endif;?>">
						<div class="title-package ">
							<?php if ($column['SubscriptionPackage']['recommended']): ?>
								<span class="recommend-label"><?php echo __('Recommend');?></span>
							<?php endif;?>
							...
							<select class="package_<?php echo $type?>_<?php echo $column['SubscriptionPackage']['id'];?>">
			...
			<tr>
				...
				<td>
					<div class="package-btn">
						<a href="javascript:void(0);" ref="<?php echo $package['SubscriptionPackage']['id'];?>" class="button_select"><?php echo __('Select');?></a>
					</div>
				</td>
				<?php endforeach;?>			
			</tr>
		...
		<div class="visible-xs">
			<div>
				...
				<?php foreach ($columns as $column): ?>
					...
					<?php $package_exit[] = $column;?>
					<div class="package_mobile <?php if ($column['SubscriptionPackage']['recommended']): ?>plan-recommend<?php endif;?>">
						<div class="title-package ">
							<?php if ($column['SubscriptionPackage']['recommended']): ?>
								<span class="recommend-label"><?php echo __('Recommend');?></span>
							<?php endif;?>
						...
						<?php endforeach;?>
						<div class="package-btn">
							<a href="javascript:void(0);" ref="<?php echo $column['SubscriptionPackage']['id'];?>" class="button_select"><?php echo __('Select');?></a>
						</div>
					</div>					
				<?php endforeach;?>
			</div>			
